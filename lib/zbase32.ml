...
let rec to_string s =
  ...
  | _ ->
    let n = String.length s in
    let rec process_input_c buf (i, (acc, pbits)) =
        if i < n then
          if not (can_skip s.[i]) then
            match ab32 s.[i] with
            | -1 -> process_input_c buf ((i + 1), (acc, pbits))
            | c -> process_input_c buf ((i + 1), (append_b32 buf c acc pbits))
          else
            process_input_c buf ((i + 1), (acc, pbits))
        else
          ( if pbits > 0 then Buffer.add_char buf ((acc land 255) |> Char.chr) ;
          n, (0, 0) )
    in
    with_buffer (1 + n/8*5) process_input_c (0, (0, 0))
